...
function guardar_administrador(){
 ...
 if (confirm("¿Esta seguro que desea continuar con el registro?")){
	...
	if(check(document.form.txtcontrasena, 'contraseña'))
	if(check(document.form.txtvericontrasena, 'verificar contraseña'))
	boolokay=true
	if(boolokay){
	  document.form.op.value=5;
...
function guardar_medico(){
 ...
 if (confirm("¿Esta seguro que desea continuar con el registro?")){
	 if(check(document.form.cimedico, 'Cédula de identidad'))
	 ...
	 if(check(document.form.beeper, 'Número de beeper'))
	 if(checkCombo(document.form.especialidad,'Especialidad'))
	 if(check(document.form.nrocolegiado, 'Número de colegiado'))
...
alert (ind);
    ...
    var validartele1 = new RegExp (/^\([0]{1}[1-9]{3}\)[0-9]{3}[-][0-9]{4}$/);  
     ...
     if(check(document.form.strtelf_uno_consultorio, 'Teléfono Principal'))
      if (!document.form.strtelf_uno_consultorio.value.match(validartele1)){   
        alert('Error: Debe ingresar un numero de Telefono Principal válido... ver el ejemplo');
    ...
    if (document.form.strtelf_dos_consultorio.value.length){
         if (!document.form.strtelf_dos_consultorio.value.match(validartele1)) {
             alert('Error: Debe ingresar un numero de Telefono Secundario Válido...ver el ejemplo ');
    ...
    if (document.form.strtelf_tres_consultorio.value.length){
         if (!document.form.strtelf_tres_consultorio.value.match(validartele1)) {
             alert('Error: Debe ingresar un numero de Telefono  Válido... ver el ejemplo');
    ...
    if (document.form.strnro_fax.value.length){
         if (!document.form.strnro_fax.value.match(validartele1)) {
             alert('Error: Debe ingresar un numero de Fax Válido... ver el ejemplo');
